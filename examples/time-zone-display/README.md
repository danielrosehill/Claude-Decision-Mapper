...
| [15](turns/15-sourcing-search-keywords.md) | Sourcing search keywords for panel + controller kit | Panel-first → matched-kit-search → controller-alone-fallback workflow locked. Common controller chipsets named (M.NT68676, RTD2660). Six-point compat checklist captured. Waveshare flagged as +$40–60 painless escape hatch |
| [16](turns/16-orange-pi-usb-c-dp-alt-mode.md) | Orange Pi USB-C DP Alt Mode support | Capability mapped: present on RK3588 family + RK3399, absent on older Allwinner/RISC-V, *absent on Pi 5/CM5*. Decision: ignore DP Alt Mode; lock HDMI-for-video + USB-C-for-power as two-cable topology. One-cable USB-C-to-monitor rejected for PD/driver fragility |
| [17](turns/17-open-frame-industrial-monitors.md) | Open-frame industrial monitors | New product category surfaced: Beetronics, Faytech, Iiyama ProLite TF, Lilliput, AOpen DT, Elo, Mimo, GVision. Beetronics 15.6" (~$350) + Pi case + VHB (~$400 total) promoted as front-runner — beats portable monitor on 24/7 rating, beats Tier-C integrated on price. Search-channel rule: kiosk channel ≠ consumer-monitor channel |
| [18](turns/18-keyword-vocabulary-locked.md) | Keyword vocabulary locked | Three-term set locked for kiosk-channel sourcing: `industrial monitor` / `open-frame monitor` / `embedded monitor`. Anti-pattern keywords (Android tablet / signage tablet / smart display) noted to keep searches from drifting into the turn-11-vetoed channel. Listing-level discriminator: HDMI+DC-only = right; OS/RAM/CPU mentions = tablet, wrong |
| [19](turns/19-industry-standard-monitor.md) | "Industry standard" monitor for wall/panel mount | No single standard. Elo = brand-consensus default (1502L iconic). 7-point spec convention captured (VESA 75/100, HDMI, 12V DC or universal AC, ≥250 nits, IP65/54, 24/7 duty, open-frame/zero-bezel). Only ISO-grade actual standard: VESA FDMI. One-line in-category spec captured as reusable RFP language |
